...
//	boolean isLockInput;
	...
	public ChargeCardRightAbstractLayout(Activity context, PayChannel channelMessage, ChargeData charge) {
		super(context);
...
//			marquee.setText(channelMessage.notice);
		...
		mPaymentNum1.setAdapter(new PaymentListAdapterSCard(YouaiAppService.mPayChannelsCard, mContext));
		...
		mPaymentNum1.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
                          ...
                          Flag.positionCard = position;
                          Flag.positionCardList = 0;

			for (int i = 0; i < parent.getCount(); i++) {
					PaymentListAdapterSCard.ChagerLayout layout = (PaymentListAdapterSCard.ChagerLayout) parent.getChildAt(i);
					if (position == i) {// 当前选中的Item改变背景颜色
						layout.setBackgroundDrawable(BitmapCache
								.getNinePatchDrawable(mContext,
										"grid_click_it.9.png"));
						layout.setPadding(0, DimensionUtil.dip2px(mContext, 7), 0,
			    	    ...
			    	    /*num = YouaiAppService.mPayChannelsCard.get(position).selectMoney.split(",");
						Flag.payTypeFlag = YouaiAppService.mPayChannelsCard.get(position).paymentId;
						Flag.payTypeFlagCard = YouaiAppService.mPayChannelsCard.get(position).paymentId;*/

					    num = YouaiAppService.mPayChannelsCard.get(position).selectMoney.split(",");
				        int paymentId =  YouaiAppService.mPayChannelsCard.get(position).paymentId;
                          Flag.payTypeFlag = paymentId;
                          Flag.payTypeFlagCard = paymentId;
		...
		mPaymentNum.setOnItemClickListener(new OnItemClickListener() {

			@Override
			public void onItemClick(AdapterView<?> adpater, View view,
                                                int position, long id) {
                          Flag.positionCardList = position;
				for (int i = 0; i < adpater.getCount(); i++) {
					...
					if (position == i) {// 当前选中的Item改变背景颜色
						...
						layout.textView.setTextColor(0xff758ba6);
						layout.setBackgroundDrawable(BitmapCache
								.getNinePatchDrawable(mContext, "grid_click.9.png"));
					}

					layout.setPadding(DimensionUtil.dip2px(mContext, 0),
                                                          DimensionUtil.dip2px(mContext, 7),
		...
		num = YouaiAppService.mPayChannelsCard.get(0).selectMoney.split(",");
	     setChannelMessages();
	}
